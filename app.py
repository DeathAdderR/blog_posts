...
# don't break my heart bb 
app.secret_key = 'abc'


# test profile for postman
# {
#     "username": "Joe",
#     "password": '123',
#     "email": "email@.com"
...
# I'm going to keep this path open until the whole framework is brought together.
@app.route('/raw_entries', methods=['GET'])

def raw_entries():
    # when using foreign keys you cannot simply fetch all entries from a database. You MUST filter by a foreign key or you will receive an empty list in return from the database!!
    db.execute_query('''SELECT * FROM blog_posts WHERE user_id = ?''', (session['user_id'],))
